...
class LogSummaryDialog(QDialog):
    def __init__(self, parent, sid=None, systray=False):
        ...
        """
        if systray:
            super(LogSummaryDialog, self).__init__()
        else:
            super(LogSummaryDialog, self).__init__(parent)
    ...
    def log_summary(self):
        ...
        option_list = qttools.QListView()
        self.comboFilter = QComboBox(self)
        main_layout.addWidget(self.comboFilter, 1)
        self.comboFilter.currentIndexChanged.connect(self.comboFilterChanged)
        ...
        self.comboProfiles = qttools.ProfileCombo(self)
        log = snapshotlog.SnapshotLog(
            self.config, self.comboProfiles)
        lines = []
        for line in log.get_all(decode=self.decode):
            lines.append(line)
        # self.txtLogView.setPlainText(
        #     '\n'.join(lines[0]['body']))
